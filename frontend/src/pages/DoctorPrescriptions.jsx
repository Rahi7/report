...
function DoctorPrescriptions() {
  ...
  const handleMedicationChange = (index, field, value) => {
    const updatedMeds = [...form.medications];
    updatedMeds[index][field] = value;
    setForm((prev) => ({ ...prev, medications: updatedMeds }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // Convert medications array to string for 'treatment' field
    const treatment = form.medications
      .map(
        (med, i) =>
...
// console.log("Doctor user from store:", user);
    ...
    <div className="max-w-4xl mx-auto mt-8 bg-white p-6 rounded shadow">
      ...
      {user && (
        <p className="mb-4 text-sm text-gray-600">
          ...
          Logged in as: <strong>Dr. {user.name}</strong>
        </p>
      )}

      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label className="block mb-1 font-medium">Patient Aadhar Number</label>
          <input
            type="text"
            name="patientId"
            value={form.patientId}
