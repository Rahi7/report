...
//   const fetchPrescriptions = async (walletAddress) => {
//     setLoading(true);
//     setError("");

//     try {
//       const res = await fetch(`/api/v1/prescription/${walletAddress}`, {
//         method: "GET",
//         credentials: "include",
//       });
...
//       fetchPrescriptions(user.walletAddress);
//     }
//   }, [user]);
...
//       {loading ? (
//         <p className="text-blue-600">Loading prescriptions...</p>
//       ) : error ? (
...
//           {prescriptions.map((prescription) => (
//             <li key={prescription._id} className="p-4 border rounded shadow">
//               <p><strong>Date:</strong> {new Date(prescription.createdAt).toLocaleDateString()}</p>
//               <p><strong>Diagnosis:</strong> {prescription.diagnosis}</p>
//               <p><strong>Medications:</strong> {prescription.medications.join(", ")}</p>
//               <p><strong>Tests:</strong> {prescription.tests.join(", ")}</p>
...
//             <li key={prescription._id} className="p-4 border rounded shadow">
//               <p><strong>Date:</strong> {new Date(prescription.createdAt).toLocaleDateString()}</p>
//               <p><strong>Diagnosis:</strong> {prescription.diagnosis || "Not provided"}</p>
//               <p><strong>Medications:</strong> {(prescription.medications || []).join(", ") || "None"}</p>
//               <p><strong>Tests:</strong> {(prescription.tests || []).join(", ") || "None"}</p>
//               <p><strong>Doctor's Notes:</strong> {prescription.notes || "No notes"}</p>
//             </li>
//           ))}
//         </ul>
//       )}
...
function PatientPrescription() {
  ...
  // console.log("createdAt:", fetchPrescriptions);
      ...
      {loading ? (
        ...
        <ul className="space-y-4">
          {prescriptions.map((prescription) => (
            <li key={prescription._id} className="p-4 border rounded shadow">
              <p><strong>Date:</strong> {new Date(prescription.createdAt).toLocaleDateString()}</p>
              <p><strong>Patient ID:</strong> {prescription.patientId || "No remarks"}</p>
              <p><strong>Doctor ID:</strong> {prescription.doctorId || "No remarks"}</p>
              <p><strong>Diagnosis:</strong> {prescription.diagnosis || "Not provided"}</p>
              <p><strong>Treatment:</strong> {prescription.treatment || "No treatment specified"}</p>
              <p><strong>Remarks:</strong> {prescription.remarks || "No remarks"}</p>
              {/* <p><strong>Aadhaar:</strong> {prescription.aadhaar || "N/A"}</p> */}
            </li>
          ))}
        </ul>
      )}
