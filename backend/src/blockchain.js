...
export const addPrescriptionOnChain = async (aadhaar, diagnosis, treatment, remarks) => {
  try {
    ...
    console.log("📤 Sent prescription tx:", tx.hash);

    await tx.wait();
    console.log("✅ Prescription transaction mined");
    return tx.hash;
  } catch (error) {
    console.error("❌ addPrescriptionOnChain failed:", error);
    throw error;
  }
...
export const getPrescriptionsFromChain = async (walletAddress) => {
  try {
    ...
    console.error("❌ getPrescriptionsFromChain failed:", error);
    throw error;
  }
};